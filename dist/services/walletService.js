...
async function ensureUserAndWallet(discordId, guildId, username) {
    ...
    const config = await (0, guildConfigService_1.getGuildConfig)(guildId);
    user = await prisma_1.default.user.upsert({
        where: { discordId_guildId: { discordId, guildId } },
        update: { username },
        create: {
            discordId,
...
async function transferMoney(fromDiscordId, toDiscordId, amount, guildId) {
    ...
    await prisma_1.default.$transaction([
        prisma_1.default.wallet.update({ where: { id: fromUser.wallet.id }, data: { balance: { decrement: amount } } }),
        prisma_1.default.transaction.create({ data: { walletId: fromUser.wallet.id, amount: -amount, type: "transfer_sent", meta: { to: toDiscordId } } }),
        prisma_1.default.wallet.update({ where: { id: toUser.wallet.id }, data: { balance: { increment: amount } } }),
        prisma_1.default.transaction.create({ data: { walletId: toUser.wallet.id, amount: amount, type: "transfer_recv", meta: { from: fromDiscordId } } })
    ]);
}
