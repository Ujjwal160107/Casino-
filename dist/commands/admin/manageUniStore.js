...
exports.handleManageUniStore = handleManageUniStore;
...
async function handleManageUniStore(message, args) {
    ...
    if (searchName) {
        ...
        if (!targetItem)
            return message.reply("Uni Store item not found.");
    }
    else {
        ...
        if (items.length === 0)
            return message.reply("Uni Shop is empty. Run `!unistore` first to seed items.");
        // Select item
    ...
    collector.on("collect", async (interaction) => {
        ...
        if (btnId === "edit_price") {
            modalId = "modal_uni_price";
            fieldId = "val_price";
            label = "New Price";
        }
        else if (btnId === "edit_desc") {
            modalId = "modal_uni_desc";
            fieldId = "val_desc";
...
//# sourceMappingURL=manageUniStore.js.map