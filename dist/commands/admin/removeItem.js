...
async function handleRemoveItem(message, args) {
    ...
    if (!targetId) {
        return message.reply({ embeds: [(0, embed_1.errorEmbed)(message.author, "Invalid Usage", "Please mention a user or provide their ID.")] });
    }
    ...
    if (!user) {
        return message.reply({ embeds: [(0, embed_1.errorEmbed)(message.author, "User Not Found", "This user does not exist in the database.")] });
    }
    const param = args.slice(1).join(" ");
    if (!param) {
        return message.reply({ embeds: [(0, embed_1.errorEmbed)(message.author, "Invalid Usage", "Provide an item name or `all`.\nExample: `,removeitem @user all` or `,removeitem @user apple`")] });
    }
    ...
    if (removeEverything) {
        const count = await prisma_1.default.inventory.deleteMany({
        ...
        return message.reply({ embeds: [(0, embed_1.successEmbed)(message.author, "Inventory Cleared", `Removed all items from ${user.username}'s inventory.`)] });
    }
    ...
    const invItems = user.inventory.filter(i => (i.shopItem.name.toLowerCase() === itemName.toLowerCase()) ||
        (i.shopItem.name.toLowerCase().includes(itemName.toLowerCase())) ||
        (i.shopItem.id.toLowerCase() === itemName.toLowerCase()));
    if (invItems.length === 0) {
        return message.reply({ embeds: [(0, embed_1.errorEmbed)(message.author, "Item Not Found", `User does not have any item matching "${itemName}".`)] });
    }
    ...
    if (targetInvItem.amount < quantity) {
        return message.reply({ embeds: [(0, embed_1.errorEmbed)(message.author, "Insufficient Quantity", `User only has ${targetInvItem.amount}x ${targetInvItem.shopItem.name}.`)] });
    }
    let newAmount = targetInvItem.amount - quantity;
    if (newAmount <= 0) {
        await prisma_1.default.inventory.delete({ where: { id: targetInvItem.id } });
    }
    else {
    ...
    return message.reply({ embeds: [(0, embed_1.successEmbed)(message.author, "Item Removed", `Removed ${quantity}x ${targetInvItem.shopItem.name} from ${targetUsername}.`)] });
}
