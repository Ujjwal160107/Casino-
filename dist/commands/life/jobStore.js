...
exports.JOB_ITEMS = void 0;
exports.handleJobStore = handleJobStore;
...
async function handleJobStore(message, args) {
    ...
    for (const item of exports.JOB_ITEMS) {
    ...
    if (args.length > 0 && (args[0].toLowerCase() === "buy")) {
        ...
        if (!query)
            return message.reply("What do you want to buy?");
        ...
        if (!item || item.name.toLowerCase() !== query.toLowerCase() && !item.name.toLowerCase().includes(query.toLowerCase())) {
            ...
            if (!item)
                return message.reply("Item not found in the Job Store.");
        }
        ...
        if (!user || user.wallet.balance < item.price) {
            return message.reply(`${branding_1.Mascot.Emotes.Fail} You cannot afford **${item.name}** (${(0, format_1.fmtCurrency)(item.price, config.currencyEmoji)}).`);
        }
        // Check if unique/passive owned
        if (!item.consumable) {
            const owned = user.inventory.find(inv => inv.shopItemId === item.id);
            if (owned)
                return message.reply(`${branding_1.Mascot.Emotes.Think} You already own a **${item.name}**.`);
        }
        await prisma_1.default.$transaction([
            ...
            prisma_1.default.inventory.upsert({
        ...
        return message.reply(`${branding_1.Mascot.Emotes.Success} You bought **${item.name}** for **${(0, format_1.fmtCurrency)(item.price, config.currencyEmoji)}**!`);
    }
    // --- DISPLAY STORE ---
    ...
    if (consumables.length > 0) {
        embed.addFields({
            name: "🥤 Consumables",
            value: consumables.map(i => `**${i.name}** — ${(0, format_1.fmtCurrency)(i.price, config.currencyEmoji)}\n*${i.description}*`).join("\n\n")
        });
    }
    if (gears.length > 0) {
        embed.addFields({
            name: "⚒️ Gear (Permanent)",
            value: gears.map(i => `**${i.name}** — ${(0, format_1.fmtCurrency)(i.price, config.currencyEmoji)}\n*${i.description}*`).join("\n\n")
        });
    }
    message.reply({
        embeds: [embed],
