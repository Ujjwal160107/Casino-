...
async function handleFamily(message, args = []) {
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .addFields({ name: "💍 Partner", value: spouseName, inline: true }, { name: "❤️ Affection", value: `${marriage.affection}`, inline: true }, { name: "🏦 Joint Savings", value: `${marriage.jointBalance.toLocaleString()}`, inline: true }, { name: "📅 Married Since", value: `<t:${Math.floor(new Date(marriage.marriedAt).getTime() / 1000)}:R>`, inline: true });
    if (spouseUser) {
