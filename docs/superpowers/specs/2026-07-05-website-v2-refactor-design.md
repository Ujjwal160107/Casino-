...
> **Revision (2026-07-06, user feedback):** Fortuna is a LIFE SIMULATOR bot — casino is one system among nine. The landing page must lead with the life sim (degrees → careers → credit → property → marriage), cover ALL nine systems (economy, bank & credit, casino, jobs & careers, education, items & shop, hunting & animals, investments, life & social), and use the real game art from `src/assets` (copied to `dashboard/public/art/`, kebab-case names): hero mockup = `!profile` (shows career/education/spouse/net worth — the life-sim breadth), a nine-module showcase grid with real asset art replacing the 2×2 WhatYouDo, feature splits reordered life-first (life sim → bank → empire/property ladder with shack→house→mansion→private-island art → casino last), and life-sim copy throughout ("Study. Work. Marry. Go broke anyway."). Sections below amended accordingly.
