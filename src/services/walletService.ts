import prisma from "../utils/prisma"; import { getGuildConfig } from "./guildConfigService"; const userIdCache = new Map<string, string>(); export async function ensureUserAndWallet(discordId: string, guildId: string, username: string) {
  const cacheKey = `${discordId}-${guildId}`; if (userIdCache.has(cacheKey)) { const user = await prisma.user.findUnique({ where: { discordId_guildId: { discordId, guildId } }, include: { wallet: true } }); if (user && user.wallet) return user; } const user = await prisma.user.upsert({
    where: { discordId_guildId: { discordId, guildId } }, update: { username }, create: {
      discordId,
      guildId,
      username,
      wallet: { create: { balance: 1000 } }
    }, include: { wallet: true }
  }); userIdCache.set(cacheKey, user.id); return user;
