...
export const fmtCurrency = (amount: number, _emoji: string = GLOBAL_CURRENCY_EMOJI) => {
  if (amount >= 2147483647) return `${GLOBAL_CURRENCY_EMOJI} ∞`;
  return `${GLOBAL_CURRENCY_EMOJI} ${amount.toLocaleString("en-US")}`;
};
