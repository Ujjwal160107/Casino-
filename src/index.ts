...
client.once("ready", async () => {
  ...
  // Registered globally rather than per guild: one API call instead of one per
  // guild, which matters at 100+ guilds and avoids rate limiting every boot.
  // Global commands can take up to an hour to appear the first time; after
  // that, changes propagate immediately.
  const rest = new REST({ version: "10" }).setToken(token);
  try {
    const body = SPECS.map((spec) => buildCommand(spec).toJSON());
...
client.on("interactionCreate", async (interaction: Interaction) => {
  try {
    if (interaction.isChatInputCommand()) {
      return await handleSlashCommand(client, interaction as ChatInputCommandInteraction);
    }
