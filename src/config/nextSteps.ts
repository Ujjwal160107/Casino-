...
const NEXT_STEPS: Record<string, string> = {
    ...
    crime_success: "Bank the payout with `{p}deposit`; if attention is climbing, check `{p}heat`",
    crime_jailed: "Check `{p}jail`, pay `{p}bail` to get out early",
    rob_success: "Bank the loot with `{p}deposit`; manage criminal attention in `{p}heat`",
