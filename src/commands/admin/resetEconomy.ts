...
export async function handleResetEconomy(message: Message, args: string[]) {
  try {
    ...
    if (token !== "confirm") {
      const embed = errorEmbed(message.author, "Confirmation Required", `This will wipe wallets, banks, transactions and audits. Run \`${prefix}reseteconomy confirm\` to proceed.`);
